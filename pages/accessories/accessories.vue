...
export default{
	...
	setup(props, context) {
		...
		const {accessoriesCategory,accessoriesSubCategory} = storeToRefs(mainStores);
		//탑높이를 지정부분
	...
	methods:{
		...
		subMenuList(id){
			...
			uni.setStorageSync('accessoriesTopSubMenu',JSON.stringify(this.rightSubMenu));
			uni.navigateTo({
			        url: `/pages/list/accessoriesList?id=${id}` // 이동할 페이지 경로
			});
		}
	},
	async onLoad() {
