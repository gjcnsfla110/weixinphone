...
const _sfc_main = {
  ...
  methods: {
    async fetchData(subMenuId, page, isRefresh = false) {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/list/list.vue:106", error);
        this.loadingStatus = "noMore";
  ...
  onLoad(op) {
    ...
    this.subMenus = JSON.parse(common_vendor.index.getStorageSync("topSubMenu"));
  }
};
if (!Array) {
  ...
  return {
    ...
    c: common_vendor.f($setup.itemList, (item, k0, i0) => {
      return common_vendor.e({
        ...
        d: common_vendor.t(item.storage),
        e: item.type == "二手商品"
      }, item.type == "二手商品" ? {
        f: common_vendor.t($setup.formattedPrice(item.price2))
      } : {
        g: common_vendor.t($setup.formattedPrice(item.price1))
      }, {
        h: common_vendor.o(($event) => item.type == "二手商品" ? $options.clickOldItemDetail(item.id) : $options.clickNewItemDetail(item.id))
      });
    }),
    d: common_vendor.p({
