...
const _sfc_main = {
  ...
  async onLoad(op) {
    try {
      ...
      common_vendor.index.__f__("error", "at pages/itemPage/newItemPage.vue:169", "onLoad 에러:", error);
    }
  }
};
if (!Array) {
  ...
  (_component_CustomNav + _component_SwiperImg + _component_OtherGoods + _easycom_uni_popup2)();
}
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    s: common_vendor.f($setup.item.content, (i, k0, i0) => {
      return {
        a: i
      };
    }),
    t: common_vendor.t($setup.spec.model),
    v: common_vendor.t($setup.spec.cpu),
    w: common_vendor.t($setup.spec.camera),
    x: common_vendor.f(JSON.parse($setup.spec.ram), (i, k0, i0) => {
      return {
        a: common_vendor.t(i)
      };
    }),
    y: common_vendor.f(JSON.parse($setup.spec.storage), (i, k0, i0) => {
      return {
        a: common_vendor.t(i)
      };
    }),
    z: common_vendor.t($setup.spec.display),
    A: common_vendor.t($setup.spec.battery),
    B: common_vendor.t($setup.spec.water),
    C: common_vendor.t($setup.spec.weight),
    D: common_vendor.t($setup.spec.type),
    E: common_vendor.t($setup.spec.color),
    F: common_vendor.sr("specPopup", "edb82754-3"),
    G: common_assets._imports_0$2,
    H: common_assets._imports_7,
    I: common_vendor.sr("servicePopup", "edb82754-4")
  });
}
